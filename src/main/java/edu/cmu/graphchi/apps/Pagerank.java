...
public class Pagerank implements GraphChiProgram<Float, Float> {
    ...
    public void update(ChiVertex<Float, Float> vertex, GraphChiContext context)  {
        if (context.getIteration() == 0) {
            ...
            //The dumping factor d, represents the probability that a random surfer of the graph
            //moves from a user to another; it is usually set to 0.85.
            vertex.setValue(0.15f + 0.85f * sum);
        }

        /* Write my value (divided by my out-degree) to my out-edges so neighbors can read it. */
        //The idea is to decrease certain amount of the degree for the seed nodes that meets the 
        //intuition for degree discount.
        //But how much to reduce? & how to reduce on the seed nodes?
     ...
     * @throws IOException
     */
    //To preprocess the graph, the need to call sharder program.
    //For example, if the graph has floating input values. 
    ...
    public static void main(String[] args) throws  Exception {
        ...
        engine.run(new Pagerank(), 1);//Runs the program for the number of iterations
